...
########### ShuffleFormerBlock #############
class BasicShuffleBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff', kernelsize=5,
                 sample_num=10, repeat = 4):
        super(BasicShuffleBlock, self).__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        self.win_size = win_size
        self.mlp_ratio = mlp_ratio
        self.token_mlp = token_mlp
        self.PEG = nn.Conv2d(dim, dim, kernel_size=kernelsize, stride=1, padding=kernelsize//2,
                                         groups=dim)
    ...
    def forward(self, x, mask=None):
        ...
        x = x.view(B, H, W, C)
        y = x + self.PEG(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        x = y.view(B, L, C)
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        ...
        if not self.training:
            out = torch.stack(shuffle_x.chunk(self.repeat, dim = 0), dim=0)
            shuffle_x = torch.mean(out, dim=0)
...
class BasicShuffleLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads, win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear', token_mlp='ffn',
                 modulator=False, cross_modulator=False, repeat=8):

        super(BasicShuffleLayer, self).__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.depth = depth
        ...
        for i in range(depth):
            if i % 2:
                block.append(BasicShuffleBlock(dim=dim, input_resolution=input_resolution,
                                      num_heads=num_heads, win_size=win_size,
                                      mlp_ratio=mlp_ratio,
                                      qkv_bias=qkv_bias, qk_scale=qk_scale,
                                      drop=drop, attn_drop=attn_drop,
                                      drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                                      norm_layer=norm_layer, token_projection=token_projection, token_mlp=token_mlp, repeat=repeat))
            else:
                block.append(BasicLocalBlock(dim=dim, input_resolution=input_resolution,
                                      num_heads=num_heads, win_size=win_size,
                                      shift_size=0,
                                      mlp_ratio=mlp_ratio,
                                      qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class ShuffleFormer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 dowsample=Downsample, upsample=Upsample, modulator=False,
                 cross_modulator=False, repeat=1, **kwargs):
        super(ShuffleFormer, self).__init__()
        ...
        # Encoder
        self.encoderlayer_0 = BasicShuffleLayer(dim=embed_dim,
                                                ...
                                                token_projection=token_projection, token_mlp=token_mlp, repeat=repeat)
        self.dowsample_0 = dowsample(embed_dim, embed_dim * 2)
        self.encoderlayer_1 = BasicShuffleLayer(dim=embed_dim * 2,
        ...
        self.encoderlayer_2 = BasicShuffleLayer(dim=embed_dim * 4,
        ...
        self.encoderlayer_3 = BasicShuffleLayer(dim=embed_dim * 8,
        ...
        # Bottleneck
        self.conv = BasicShuffleLayer(dim=embed_dim * 16,
        ...
        self.decoderlayer_0 = BasicShuffleLayer(dim=embed_dim * 16,
                                                output_dim=embed_dim * 16,
                                                input_resolution=(img_size // (2 ** 3),
                                                                  img_size // (2 ** 3)),
                                                depth=depths[5],
                                                ...
                                                modulator=modulator, cross_modulator=cross_modulator, repeat=repeat)
        self.upsample_1 = upsample(embed_dim * 16, embed_dim * 4)
        self.decoderlayer_1 = BasicShuffleLayer(dim=embed_dim * 8,
                                                output_dim=embed_dim * 8,
                                                input_resolution=(img_size // (2 ** 2),
                                                                  img_size // (2 ** 2)),
                                                depth=depths[6],
        ...
        self.decoderlayer_2 = BasicShuffleLayer(dim=embed_dim * 4,
                                                output_dim=embed_dim * 4,
                                                input_resolution=(img_size // 2,
                                                                  img_size // 2),
                                                depth=depths[7],
        ...
        self.decoderlayer_3 = BasicShuffleLayer(dim=embed_dim * 2,
                                                output_dim=embed_dim * 2,
                                                input_resolution=(img_size,
                                                                  img_size),
                                                depth=depths[8],
...
if __name__ == "__main__":
    ...
    arch = ShuffleFormer
    model_restoration = ShuffleFormer(img_size=input_size, embed_dim=32, win_size=8, token_projection='linear', token_mlp='leff',
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1], dd_in=3, repeat=repeat_num)
